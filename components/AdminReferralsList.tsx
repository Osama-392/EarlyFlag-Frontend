...
export default function AdminReferralsList({ range }: { range?: '1d' | '7d' | '30d' | 'all' }) {
        ...
        <div className="flex items-center gap-6 mt-6">
          {tabs.map(tab => (
            <button
              ...
              className={`pb-3 text-sm font-bold border-b-2 transition-colors flex items-center gap-1.5 ${activeTab === tab.id
 ...
 <span className="text-black dark:text-white font-bold"> - {referral.subject}</span>
                      ...
                      {referral.class_name && (
                        <span className={`px-2 py-0.5 rounded-md text-xs font-bold border ${isRedFlag ? 'bg-red-50 text-red-600 border-red-100 dark:bg-red-900/30 dark:text-red-400 dark:border-red-900/50' : 'bg-blue-50 text-blue-600 border-blue-100 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-900/50'}`}>
                          {referral.class_name}
                        </span>
                      )}
                      {isRedFlag ? (
